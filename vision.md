...
| 1.11 | 2026-06-16 | PRD lookup-usuario-musica-captacao (D02) | Captação substitui "usuário de música (texto livre)" por `usuarioMusicaId` + `usuarioMusicaNome`. Arrecadação publica eventos `arrecadacao.usuario-musica.criado`/`atualizado` para projeção local na Identificação via event-driven ACL. |
