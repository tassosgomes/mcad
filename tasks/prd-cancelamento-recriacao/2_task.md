...
- [ ] 2.1 Criar `DistribuicaoEventConsumer` como BackgroundService
- [ ] 2.2 Declarar queue `identificacao.distribuicao.rol.processado`, bind ao exchange `distribuicao` com routing key `distribuicao.rol.processado`
- [ ] 2.3 Ao receber: deserializar evento, buscar captação por ID, chamar `MarcarDistribuicaoProcessada()`, save
- [ ] 2.4 Idempotência: se já processada, ignorar (log info)
- [ ] 2.5 Error handling: nack com requeue em caso de erro
- [ ] 2.6 Logging: log info para processamento bem-sucedido, log warning para captação não encontrada, log error para exceções
