...
- [ ] 1.1 Adicionar `public long Codigo { get; private set; }` nas 4 entidades
- [ ] 1.2 Em cada Configuration: `builder.Property(e => e.Codigo).HasDefaultValueSql("nextval('cadastro.seq_{tabela}_codigo')").ValueGeneratedOnAdd();` + `builder.HasIndex(e => e.Codigo).IsUnique().HasDatabaseName("uq_{tabela}_codigo");`
- [ ] 1.3 AssociacaoSeed: adicionar Codigo 1-7 nos registros
- [ ] 1.4 Gerar migration. A migration deve incluir:
...
- [ ] `SELECT Codigo FROM cadastro.associacoes ORDER BY Codigo` → 1,2,3,4,5,6,7
- [ ] `INSERT INTO cadastro.titulares (...) RETURNING "Codigo"` → retorna inteiro > 0
- [ ] Unique constraint funciona (tentativa de duplicar Codigo falha)
