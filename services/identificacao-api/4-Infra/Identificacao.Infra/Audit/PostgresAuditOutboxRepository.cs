...
public sealed class PostgresAuditOutboxRepository : IAuditOutboxRepository
{
    ...
    public PostgresAuditOutboxRepository(IdentificacaoDbContext context, JsonSerializerOptions jsonOptions)
    {
        _context = context;
    ...
    public async Task SaveAsync(AuditEvent auditEvent, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            INSERT INTO identificacao.audit_outbox (
    ...
    public async Task<IReadOnlyList<AuditOutboxRecord>> LockPendingAsync(
        string lockOwner, int batchSize, TimeSpan lockTtl, CancellationToken cancellationToken = default)
    {
        ...
        const string selectSql = """
            ...
            FROM identificacao.audit_outbox
    ...
    public async Task MarkSentAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            UPDATE identificacao.audit_outbox
            SET status = 'SENT', sent_at_utc = NOW(), locked_until_utc = NULL, lock_owner = NULL
            WHERE id = @id
            """;
    ...
    public async Task MarkFailedAsync(string id, string? errorMessage, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            UPDATE identificacao.audit_outbox
            SET status = 'FAILED', retry_count = retry_count + 1, last_error = @last_error,
                available_at_utc = NOW() + INTERVAL '60 seconds',
                locked_until_utc = NULL, lock_owner = NULL
            WHERE id = @id
            """;
    ...
    private async Task LockRecordAsync(
        ...
        string id, string lockOwner, DateTime lockedUntil, CancellationToken cancellationToken)
    {
        const string sql = """
            UPDATE identificacao.audit_outbox
            SET status = 'SENDING', locked_until_utc = @locked_until_utc, lock_owner = @lock_owner
