...
public sealed class IdentityUserEventConsumer : BackgroundService
{
    ...
    private const string DefaultQueue = "cadastro.identity.users";
        ...
        ILogger<IdentityUserEventConsumer> logger)
    {
        ...
        _logger = logger;
        _rabbitUrl = RabbitMqPublisher.ResolveRabbitMqUrl(configuration);
    ...
    private static Task UpsertAsync(CadastroDbContext context, IdentityUserEvent identityEvent, CancellationToken ct)
    {
        const string sql = """
            INSERT INTO cadastro.usuarios_identidade (
                logto_user_id,
                username,
                display_name,
                email,
