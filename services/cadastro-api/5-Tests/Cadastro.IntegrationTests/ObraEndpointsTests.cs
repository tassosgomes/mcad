...
public class ObraEndpointsTests : IClassFixture<CadastroApiFactory>
{
    ...
    [Fact]
    public async Task Get_BuscarObraPorId_BloqueadaRetornaJustificativa()
    {
        var postRes = await _client.PostAsJsonAsync("/api/v1/obras", new CriarObraCommand("Obra Bloqueada", null, "MUSICAL", "Rap"));
        var created = await postRes.Content.ReadFromJsonAsync<ObraResponse>();

        var bloquearRes = await _client.PostAsJsonAsync(
            $"/api/v1/obras/{created!.Id}/bloquear",
            new BloquearObraRequest("Conflito de titularidade em análise"));

        bloquearRes.StatusCode.Should().Be(HttpStatusCode.OK);

        var getRes = await _client.GetAsync($"/api/v1/obras/{created.Id}");
        getRes.StatusCode.Should().Be(HttpStatusCode.OK);

        var obra = await getRes.Content.ReadFromJsonAsync<ObraResponse>();
        obra!.BloqueioJustificativa.Should().Be("Conflito de titularidade em análise");
    }
    ...
    [Fact]
    public async Task Post_DepurarObra_DeveCopiarTitularidadesParaNovaObra()
    {
        var postRes = await _client.PostAsJsonAsync("/api/v1/obras", new CriarObraCommand("Para Depurar Com Titularidades", null, "MUSICAL", null));
        var created = await postRes.Content.ReadFromJsonAsync<ObraResponse>();
        ...
        var depurarRes = await _client.PostAsJsonAsync(
            $"/api/v1/obras/{created.Id}/depurar",
            new DepurarObraRequest("Depurada Copiada", "MUSICAL", "Sub", "Pop"));

        depurarRes.StatusCode.Should().Be(HttpStatusCode.Created);

        var depuracao = await depurarRes.Content.ReadFromJsonAsync<DepuracaoResponse>();
        depuracao.Should().NotBeNull();

        var titularidadesRes = await _client.GetAsync($"/api/v1/obras/{depuracao!.NovaObra.Id}/titularidades");
        titularidadesRes.StatusCode.Should().Be(HttpStatusCode.OK);

        var titularidades = await titularidadesRes.Content.ReadFromJsonAsync<TitularidadesResponse>();
        titularidades.Should().NotBeNull();
        titularidades!.Titularidades.Should().HaveCount(1);
        titularidades.SomaPercentual.Should().Be(100.0m);
        titularidades.SomaCompleta.Should().BeTrue();
        titularidades.Titularidades.First().Categoria.Should().Be("AUTOR");
        titularidades.Titularidades.First().Percentual.Should().Be(100.0m);
    }
