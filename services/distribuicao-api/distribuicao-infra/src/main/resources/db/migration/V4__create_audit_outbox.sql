CREATE TABLE IF NOT EXISTS distribuicao.audit_outbox (
...
CREATE INDEX IF NOT EXISTS ix_audit_outbox_pending
    ON distribuicao.audit_outbox(status, available_at_utc, created_at_utc);

CREATE INDEX IF NOT EXISTS ix_audit_outbox_event_type
    ON distribuicao.audit_outbox(event_type, created_at_utc);
