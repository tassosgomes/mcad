...
public class GenericAuditEventFactory {
    ...
    public AuditEvent userAction(
            ...
            String actionCode, String actionName, String reason,
            String screenId, String screenName,
            AuditContext context) {
        return new AuditEvent(
                UUID.randomUUID().toString(), SCHEMA_VERSION, EventType.USER_ACTION,
                OffsetDateTime.now(ZoneOffset.UTC),
                source(), actor(context), origin(context, screenId, screenName), correlation(context),
                null, security(),
                Map.of("reason", reason),
                null,
                new AuditEvent.UserAction(actionCode, actionName, context.businessContext(entityType, entityId))
        );
    }
    ...
    public AuditEvent dataChange(
            ...
            String reason, String screenId, String screenName,
            AuditContext context) {
        ...
        return new AuditEvent(
                UUID.randomUUID().toString(), SCHEMA_VERSION, EventType.DATA_CHANGE,
                OffsetDateTime.now(ZoneOffset.UTC),
                source(), actor(context), origin(context, screenId, screenName), correlation(context),
                new AuditEvent.Data(entityType, entityId, null, dataAction, before, dataAfter, changedFields),
                security(), Map.of("reason", reason), null, null
        );
    }
