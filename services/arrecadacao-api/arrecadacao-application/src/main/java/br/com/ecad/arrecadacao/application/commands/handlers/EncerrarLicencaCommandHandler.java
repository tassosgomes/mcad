...
@Component
public class EncerrarLicencaCommandHandler implements CommandHandler<EncerrarLicencaCommand, LicencaResponse> {
    ...
    public EncerrarLicencaCommandHandler(LicencaRepository licencaRepository,
    ...
    @Override
    @Transactional
    public LicencaResponse handle(EncerrarLicencaCommand cmd) {
        ...
        var historico = licenca.encerrar(cmd.justificativa(), cmd.autor());
        ...
        auditClient.publish(auditFactory.userAction(
            ...
            "ENCERRAR_LICENCA", "Encerrar licença",
            "Licença encerrada: " + cmd.justificativa(), SCREEN_ID, SCREEN_NAME, auditCtx));
        auditClient.publish(auditFactory.dataChange(
            ENTITY_TYPE, entityId, DataAction.UPDATE,
            before, LicencaAuditMapper.map(licenca),
            "Licença encerrada", SCREEN_ID, SCREEN_NAME, auditCtx));
